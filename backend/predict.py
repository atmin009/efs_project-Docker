...
def predict(request: PredictionRequest, db: Session) -> List[PredictionResponse]:
    ...
    for building_id in building_ids:
        ...
        for i in range(1, 12):
            ...
            # ถ้าเดือนติดลบหรือเป็น 0 ให้ลดปีลง และเพิ่มเดือนให้ถูกต้อง
            if prev_month <= 0:
                prev_month += 12
                prev_year -= 1
        ...
        for i, model_name in enumerate(model_names):
            try:
                ...
                # คำนวณ month_predict และ year_predict โดยเริ่มจากเดือนถัดไป
                month_predict = (month + i + 1) % 12 or 12
                year_predict = year + (month + i) // 12
